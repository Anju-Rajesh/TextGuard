    ...
    if request.method == 'POST':
        ...
        if action == 'corpus_check':
            ...
            if text_to_check.strip():
                # Use the new Dataset-Reference Logic
                ...
                analysis = PlagiarismAnalysis(
                    user_id=current_user.id,
                    source_text=f"Dataset Search (Top: {top_source})",
                ...
                flash('Please provide text or file to check against dataset.', 'info')
...
@login_required
def history():
    """Fetches and displays categorized history for the current user."""
    ai_history = AIAnalysis.query.filter_by(user_id=current_user.id).order_by(AIAnalysis.created_at.desc()).all()
    similarity_history = SimilarityAnalysis.query.filter_by(user_id=current_user.id).order_by(SimilarityAnalysis.created_at.desc()).all()
    plagiarism_history = PlagiarismAnalysis.query.filter_by(user_id=current_user.id).order_by(PlagiarismAnalysis.created_at.desc()).all()
    
    return render_template('history.html', 
                           ai_history=ai_history, 
                           similarity_history=similarity_history, 
                           plagiarism_history=plagiarism_history)
